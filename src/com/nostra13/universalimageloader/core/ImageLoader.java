...
public class ImageLoader {
	...
	//得到内存或者磁盘中的图片
	public Bitmap getExistBmp(String uri)
	{
		if(Util.IsEmpty(uri))
			return null;
		...
		ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, null, targetSize, null, null);
		LoadAndDisplayImageTask displayImageTask = new LoadAndDisplayImageTask(configuration, imageLoadingInfo, new Handler());
		
		return displayImageTask.onlyTryDisBmp();
	}
	
	public void DoReleaseImg(ImageView img)
	{
		if(img==null)
			return  ;
	...
	public String getLoadingUriForView(ImageView imageView) {
		if(cacheKeyForImageView.containsKey(imageView))
			return cacheKeyForImageView.get(imageView);
		return null;
	}
	
	public void CancelImageShow(ImageView imageView)
	{
		if(cacheKeyForImageView.containsKey(imageView))
			cacheKeyForImageView.remove(imageView);
	}
